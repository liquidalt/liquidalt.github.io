...
// ── NAV ────────────────────────────────────────────────
function goTab(id) {
  document.querySelectorAll('.ntab').forEach((t,i)=>t.classList.toggle('on',['home','race','teams','items','inventory','shop','chat','lb','dm','dating'][i]===id));
...
const DATING_PROFILES = [
  ...
  },
  {
    ...
    compatibility: () => Math.floor(Math.random() * 30) + 5,
    msg: 'my coins > your coins. just saying.'
  },
  {
    name: 'Random Person',
    age: '???',
...
let datingState = { currentIdx: 0, matches: [], viewed: new Set() };

function renderDating() {
  const el = document.getElementById('tab-dating');
  if (!el || !UC) return;
  ...
  el.innerHTML = `
    <div style="max-width:620px;margin:0 auto;padding:0 16px">
      <div style="text-align:center;margin-bottom:24px">
        <div style="font-family:'Bebas Neue',cursive;font-size:3rem;letter-spacing:6px;color:#ff6699;text-shadow:0 0 20px rgba(255,100,150,.4)">💘 LIQUIDTYPE DATING</div>
        <div style="color:var(--muted);font-size:.82rem;letter-spacing:3px;text-transform:uppercase">this is a joke. please dont actually try to date these people</div>
      </div>

      <div class="card-panel" style="border-color:rgba(255,100,150,.2);margin-bottom:16px;position:relative;overflow:hidden">
        <div style="position:absolute;top:0;left:0;right:0;height:3px;background:linear-gradient(90deg,#ff6699,#ff99cc,#ff6699)"></div>
        <div style="display:flex;gap:20px;align-items:flex-start;flex-wrap:wrap">
          <div style="font-size:5rem;line-height:1;flex-shrink:0;filter:drop-shadow(0 0 12px rgba(255,100,150,.4))">${profile.emoji}</div>
          <div style="flex:1;min-width:200px">
            <div style="font-family:'Bebas Neue',cursive;font-size:2rem;letter-spacing:3px;color:#fff">${esc(profile.name)}</div>
            <div style="color:var(--muted);font-size:.82rem;margin-bottom:8px">Age: ${profile.age}</div>
            <div style="font-size:.95rem;color:var(--text);line-height:1.6;margin-bottom:12px;font-style:italic">"${esc(profile.bio)}"</div>
            <div style="display:flex;flex-wrap:wrap;gap:6px;margin-bottom:10px">
              ${profile.interests.map(i=>`<span style="padding:3px 10px;border-radius:12px;background:rgba(255,100,150,.1);border:1px solid rgba(255,100,150,.25);font-size:.76rem;color:#ff99cc">${i}</span>`).join('')}
            </div>
            <div style="font-size:.8rem;color:var(--muted)">🚩 Dealbreaker: <span style="color:#ff6644">${esc(profile.dealbreaker)}</span></div>
          </div>
        </div>

        <div style="margin-top:18px;padding:14px;background:rgba(255,100,150,.06);border:1px solid rgba(255,100,150,.15);border-radius:10px">
          <div style="font-size:.72rem;letter-spacing:3px;text-transform:uppercase;color:rgba(255,100,150,.6);margin-bottom:6px">COMPATIBILITY WITH ${esc((UC.username||'you').toUpperCase())}</div>
          <div style="display:flex;align-items:center;gap:12px">
            <div style="flex:1;height:10px;background:rgba(255,255,255,.07);border-radius:5px;overflow:hidden">
              <div style="height:100%;width:${compat}%;background:${compatColor};border-radius:5px;transition:width 1s ease;box-shadow:0 0 8px ${compatColor}"></div>
            </div>
            <div style="font-family:'Bebas Neue',cursive;font-size:1.6rem;color:${compatColor};min-width:42px;text-align:right">${compat}%</div>
          </div>
          <div style="font-size:.8rem;color:${compatColor};margin-top:4px;letter-spacing:1px">${compatLabel}</div>
        </div>

        <div style="margin-top:14px;padding:10px 14px;background:rgba(255,255,255,.03);border-left:3px solid rgba(255,100,150,.3);border-radius:0 6px 6px 0">
          <div style="font-size:.72rem;color:var(--muted);margin-bottom:3px">${esc(profile.name)} would say:</div>
          <div style="font-size:.92rem;color:var(--text);font-style:italic">"${esc(profile.msg)}"</div>
        </div>
      </div>

      <div style="display:flex;gap:10px;justify-content:center;margin-bottom:20px;flex-wrap:wrap">
        <button onclick="datingSwipe('nope')" style="padding:14px 32px;border:2px solid #ff4444;border-radius:50px;background:rgba(255,68,68,.1);color:#ff4444;font-family:'Bebas Neue',cursive;font-size:1.4rem;letter-spacing:3px;cursor:pointer;transition:.2s" onmouseover="this.style.background='rgba(255,68,68,.2)'" onmouseout="this.style.background='rgba(255,68,68,.1)'">✕ NOPE</button>
        <button onclick="datingSwipe('like')" style="padding:14px 32px;border:2px solid #ff6699;border-radius:50px;background:rgba(255,102,153,.1);color:#ff6699;font-family:'Bebas Neue',cursive;font-size:1.4rem;letter-spacing:3px;cursor:pointer;transition:.2s" onmouseover="this.style.background='rgba(255,102,153,.2)'" onmouseout="this.style.background='rgba(255,102,153,.1)'">💘 LIKE</button>
        <button onclick="datingSwipe('super')" style="padding:14px 32px;border:2px solid #ff99cc;border-radius:50px;background:rgba(255,150,200,.1);color:#ff99cc;font-family:'Bebas Neue',cursive;font-size:1.4rem;letter-spacing:3px;cursor:pointer;transition:.2s" onmouseover="this.style.background='rgba(255,150,200,.2)'" onmouseout="this.style.background='rgba(255,150,200,.1)'">⭐ SUPER LIKE</button>
      </div>

      ${datingState.matches.length > 0 ? `
      <div class="card-panel" style="border-color:rgba(255,100,150,.2)">
        <div style="font-family:'Bebas Neue',cursive;font-size:1.2rem;letter-spacing:4px;color:#ff6699;margin-bottom:12px">💌 YOUR MATCHES (${datingState.matches.length})</div>
        ${datingState.matches.map(m=>`
          <div style="display:flex;align-items:center;gap:10px;padding:10px;background:rgba(255,100,150,.05);border:1px solid rgba(255,100,150,.12);border-radius:8px;margin-bottom:6px">
            <span style="font-size:1.6rem">${m.emoji}</span>
            <div style="flex:1">
              <div style="font-family:'Bebas Neue',cursive;letter-spacing:2px;color:#fff">${esc(m.name)}</div>
              <div style="font-size:.75rem;color:var(--muted);font-style:italic">"${esc(m.msg)}"</div>
            </div>
            <div style="font-family:'JetBrains Mono',monospace;font-size:.75rem;color:#ff6699">${m.isSuper?'⭐ SUPER LIKED':'💘 LIKED'}</div>
          </div>`).join('')}
        <div style="font-size:.76rem;color:var(--muted);text-align:center;margin-top:8px;letter-spacing:2px">NONE OF THESE ARE REAL. THIS IS A JOKE.</div>
      </div>` : ''}

      <div style="text-align:center;margin-top:18px;padding:14px;background:rgba(255,255,255,.03);border-radius:10px">
        <div style="font-size:.75rem;color:rgba(255,255,255,.2);letter-spacing:2px;line-height:1.8">
          LiquidType Dating™ is not responsible for any emotional damage caused by DePoule's compatibility score.<br>
          All profiles are fictional characters within LiquidType. Except OverControl. He's real. He's watching.<br>
          <span style="color:rgba(255,100,150,.4)">No bottlecaps were harmed in the making of this feature.</span>
        </div>
      </div>
    </div>
  `;
...
function datingSwipe(type) {
  const profile = DATING_PROFILES[datingState.currentIdx % DATING_PROFILES.length];
  if (type === 'like' || type === 'super') {
    const already = datingState.matches.find(m => m.name === profile.name);
    if (!already) {
      datingState.matches.unshift({ ...profile, isSuper: type === 'super' });
    }
    const toast = type === 'super'
      ? `⭐ Super liked ${profile.name}! They said: "${profile.msg}"`
      : `💘 Liked ${profile.name}!`;
    showToast(toast);
  } else {
    showToast(`Noped ${profile.name}. Their loss.`);
  }
  datingState.currentIdx++;
  if (datingState.currentIdx >= DATING_PROFILES.length) {
    datingState.currentIdx = 0;
    showToast("You've seen everyone. Starting over. This is fine.");
  }
  renderDating();
}
